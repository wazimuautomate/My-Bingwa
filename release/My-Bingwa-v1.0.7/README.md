...
|---|---|---|
| `My-Bingwa-v1.0.7-play.aab` | **Google Play bundle** (13.8 MB) | Play Console → Production → Create release → upload this |
| `My-Bingwa-v1.0.7-play.aab.sha256` | Checksum for the bundle (computed locally after download — the CI job does not generate one for the AAB) | Verify before uploading |
| `My-Bingwa-v1.0.7-direct.apk` | Signed APK for direct/sideload distribution (13.9 MB) | GitHub Release / direct download. **Not** for Play |
| `My-Bingwa-v1.0.7-direct.apk.sha256` | Checksum for the direct APK (from CI) | Verify before distributing |
...
- **The signing identity is byte-identical to v1.0.3 and v1.0.6**: certificate SHA-256
  ...
  `CN=My Bingwa, O=My Bingwa, L=Nairobi, C=KE` — verified with `apksigner verify
...
cause: `MyBingwaApplication.onCreate()` re-scheduled the daily engagement
...
4. **No SMS permission prompt anywhere**, and Android Settings → Apps → My Bingwa →
