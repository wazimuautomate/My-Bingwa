...
# 4. Reusable prompt for each later Claude session

For a new session, paste this and replace every bracketed value:

```text
You are implementing My Bingwa [PHASE NUMBER AND NAME].

Work only in this phase's dedicated worktree and branch:

- Worktree/folder: [ABSOLUTE OR CURRENT WORKTREE]
- Branch: [FEATURE BRANCH]
- Base: the tested main commit containing all required dependency phases

Before changing anything:

1. Read CLAUDE.md completely.
2. Read this phase in CLAUDE_KICKOFF_AND_BUILD_PHASES.md.
3. Read the relevant sections of Plan.md and design.md.
4. Read memory.md current state and latest execution entries.
5. Read [Unreleased] in CHANGELOG.md.
6. Inspect git status and preserve unrelated changes.
7. Inspect the existing implementation and tests; do not assume the phase is
   empty.

Your exact assignment:

[PASTE THE PHASE SCOPE FROM THIS DOCUMENT]

Ownership boundaries:

- You may edit: [OWNED MODULES/DIRECTORIES]
- You must not edit unless strictly required: [SHARED OR OTHER PHASE MODULES]
- Do not redesign or refactor another phase's work.
- If a shared contract is missing, stop and report the smallest contract needed
  instead of creating a competing architecture.
- Do not edit global navigation registration if this phase is assigned to
  expose route-entry contracts for the integration phase.
- Append required memory/changelog information; do not rewrite or remove other
  sessions' entries.

Requirements:

- Product behaviour follows Plan.md.
- UI and motion follow design.md exactly.
- Preserve honest payment and fulfilment language.
- Include relevant online, offline, loading, empty, error and restoration
  states.
- Add or update unit, ViewModel and Compose tests for this phase.
- Inspect light, dark, small-width, 200%-text and reduced-motion states for
  changed UI.
...
1. Run the relevant local Gradle checks.
2. Update memory.md.
3. Update CHANGELOG.md for repository changes.
4. Review the complete diff.
5. Commit coherent changes.
6. Push only this feature branch.
7. Do not merge or push main. The coordinator/integration session owns main.
8. Report the branch, commit, tests, CI state, artifact if produced, blockers
   and exact integration instructions.

If the phase cannot be completed because a required contract, credential,
business value or dependency phase is missing, do not fake it. Implement only
safe interfaces/test doubles within scope, mark the result partial and report
the exact blocker.
```

---

# 5. Coordinator integration prompt

Use this only after a set of worker branches is pushed:

```text
You are the My Bingwa integration coordinator.

Do not implement a new feature. Integrate these completed branches one at a
time from an up-to-date main:

[LIST BRANCHES AND COMMIT SHAS]

Before merging, read CLAUDE.md, Plan.md, design.md, memory.md,
CLAUDE_KICKOFF_AND_BUILD_PHASES.md and CHANGELOG.md. Inspect every worker's
reported tests and diff.

For each branch:

1. Confirm it was based on the required dependency commit.
2. Review scope and ownership violations.
3. Run its focused tests.
4. Merge without discarding another branch's work.
5. Resolve memory.md and CHANGELOG.md conflicts by preserving every factual
   entry.
6. Resolve shared-code conflicts according to Phase 1 contracts, not by keeping
   whichever side is easier.
7. Run the full test/lint/debug-assemble gate after each merge.

After all merges:

- Verify navigation and cross-feature behaviour.
- Inspect light, dark, offline, error, large-text and reduced-motion states.
- Push the integration branch.
- Run GitHub Actions.
- Identify the exact debug APK artifact.
- Ask for/record physical-phone acceptance before main is considered complete.
- Merge and push main only after required evidence and authorisation exist.

Report every merged branch, resulting commit, conflict resolution, test result,
CI result, APK artifact and remaining blocker. Never fabricate a successful
merge or build.
```
...
Claude must not invent these values or hide their absence behind mock success.
