...
You are taking over development of My Bingwa, an Android application for purchasing Safaricom Bingwa bundles (Data, SMS and Minutes).
...
Unlike most apps, users continue interacting with My Bingwa even when internet connectivity disappears.
...
My Bingwa reads only Safaricom bundle messages so it can notify you when your bundles are running low and automatically update your bundle status.
...
# MY BINGWA - PRODUCTION RELEASE IMPLEMENTATION
...
Do **not** remove or regress existing features. The goal is to make My Bingwa feel significantly more intelligent, reliable, and personal while preserving its offline-first philosophy.
