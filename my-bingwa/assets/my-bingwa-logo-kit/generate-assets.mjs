...
await fs.writeFile(
  path.join(brandDir, "my-bingwa-symbol-transparent-1024.png"),
  symbolMaster,
);

for (const size of [512, 256, 128, 64]) {
  await fs.writeFile(
    path.join(brandDir, `my-bingwa-symbol-transparent-${size}.png`),
    await transparentSymbolCanvas(size, 0.8),
  );
}

await fs.writeFile(
  path.join(brandDir, "my-bingwa-symbol-monochrome-black-1024.png"),
  await monochromeSymbol(1024, { r: 0, g: 0, b: 0 }, 0.8),
);

await fs.writeFile(
  path.join(brandDir, "my-bingwa-symbol-monochrome-white-1024.png"),
...
await fs.writeFile(
  path.join(androidDir, "my-bingwa-launcher-master-1024.png"),
  launcherMaster,
);
await fs.writeFile(
  path.join(androidDir, "my-bingwa-launcher-round-master-1024.png"),
  roundLauncherMaster,
);
...
for (const [density, size] of Object.entries(notificationSizes)) {
  ...
  await fs.mkdir(directory, { recursive: true });
  await fs.writeFile(
    path.join(directory, "ic_stat_my_bingwa.png"),
    await monochromeSymbol(size, { r: 255, g: 255, b: 255 }, 0.78),
  );
}

await fs.writeFile(
  path.join(androidDir, "my-bingwa-splash-mark-512.png"),
  await transparentSymbolCanvas(512, 0.72),
);

await fs.writeFile(
  path.join(androidDir, "my-bingwa-play-store-512.png"),
...
await fs.writeFile(path.join(previewDir, "my-bingwa-logo-kit-preview.png"), preview);
